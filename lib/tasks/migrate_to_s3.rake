...
namespace :paperclip_migration do  
  ...
  task :migrate_to_s3 => :environment do
    klasses = [:feed_site] # Replace with your real model names. If anyone wants to this could be picked up from args or from configuration.
            ...
            definitions.keys.each do |definition|
              if record.send("#{definition}_file_name")
                ...
                if File.exists?(attachment.path)
                  file_data =  File.open(attachment.path)
                  puts "Saving file: #{attachment.url} to Amazon S3..."
                  record.send("#{definition}").assign file_data
                  record.send("#{definition}").save
                else
                  puts "Can't find file: #{attachment.path} NOT MIGRATING..."
                end
              end
            end
          end
